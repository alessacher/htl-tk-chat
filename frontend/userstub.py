...
def test_connection(ip):
  """Test the Connection to specific users
...
def test_user_table(window):
  ...
  for i, (name, ip) in enumerate(test_users):
    ...
    if user_online: status_color = QColor('#80005500')
    else: status_color = QColor('#80ff3737')
...
def test_combo_box(window):
  """Adds test users to the Combo Box for recipients
